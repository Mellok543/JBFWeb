...
Чтобы сразу открыть доступ к админ-панели своему Steam-аккаунту:

```powershell
.\run-local.ps1 -AdminSteamId 76561198XXXXXXXXX
```

Используйте именно SteamID64.
...
- интеграция с VIPCore / экономикой / косметикой;
- платёжный провайдер и автоматическая выдача оплаченных товаров.


## Админ-панель

Админ-панель доступна по:

```text
http://localhost:5173/admin
```

Доступ выдаётся только Steam-аккаунтам, перечисленным в переменной:

```text
ADMIN_STEAM_IDS=76561198000000000,76561198000000001
```

После изменения переменной перезапустите backend и заново войдите через Steam.
...
- товары;
- заказы и их статусы;
- список и поиск игроков по нику / SteamID64;
- Battle Pass / кланы;
